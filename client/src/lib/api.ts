...
/**
 * Start deposit - get user's personal TRC20 address
 */
export async function startDeposit(
  userId: string
): Promise<{ tronAddress: string; qrCode: string }> {
  const response = await fetch(`${API_BASE}/deposits/start`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  if (!response.ok) {
    const error = await response.json();
    throw new Error(error.error || 'Failed to start deposit');
  }
